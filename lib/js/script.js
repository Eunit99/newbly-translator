...
newblyFnc(function () {
  ...
  var findContainerElement = function findContainerElement(searchString) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      try {
        for (var _iterator = document.querySelectorAll(_element)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var _element = _step.value;

          if (_element.textContent.includes(searchString)) {
            matches.push(_element);
